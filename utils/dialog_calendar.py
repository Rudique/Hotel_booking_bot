...
class DialogCalendar:
    """
    Класс-календарь для выбора даты пользователем
    """

    months = ["Янв", "Фев", "Мар", "Апр", "Mай", "Июнь", "Июль", "Авг", "Сен", "Окт", "Ноя", "Дек"]
    ...
    async def start_calendar(self, year: int = datetime.now().year) -> InlineKeyboardMarkup:
        """
        Корутина запускающая календарь в виде инлайн клавиатуры

        :param year: текущий год
        :return: клавиатура для выбор года
        """
    ...
    async def _get_month_kb(self, year: int):
        ...
        :param year: текущий год
        :return: клавиатура для выбор месяца
        """
    ...
    async def _get_days_kb(self, year: int, month: int):
        ...
        :param year: текущий год
        :return: клавиатура для выбор дня
        """
    ...
    async def process_selection(self, query: CallbackQuery, data: CallbackData) -> tuple:
        """
        Корутина для координации клавиатур с годом, датой и днями

        :param query: объект CallbackQuery ответ на нажатие кнопки на инлайн клавиатуре
        :param data: данные запроса
        :return: обработанные данные запроса для навигации по календарю
        """
